...
*** Multiplexer
[[file:2505211814 arduino projects-images/multiplexer.jpg]]
[[https://www.instructables.com/Arduino-Interfacing-With-CD74HC4067-16-channel-MUX/][Code from Instructables]] 
#+begin_src cpp
...
#+end_src
The values read from the pots are different from those read directly, maximum is about 800 rather than 1024 so this will need to be accounted for in the call to *scaledValue*. The output from pin 1 is different from the others, topping out at only 470ish. Maybe its a soldering fault, but there are a few spare pins in several of the proposed sub-units so it shouldn't be a problem.
*** Sending Midi
[[https://github.com/arduino-libraries/MIDIUSB/blob/master/examples/MIDIUSB_write/MIDIUSB_write.ino][MidiUSB Example]]
